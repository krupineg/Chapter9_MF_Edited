...
IMFTransform* FindEncoderTransform(IMFMediaType * in_media_type, GUID out_video_format) {
    UINT32 count = 0;
    ...
    MFT_REGISTER_TYPE_INFO info_in = { 0 };
    info_in.guidMajorType = MFMediaType_Video;
    info_in.guidSubtype = neededInputType;
...
{
    IMFTransform * pEncoder = FindEncoderTransform(in_media_type, out_video_format);
    HRESULT hr = S_OK;    
...
)
{
    ...
    inputInfo.guidMajorType = bAudio ? MFMediaType_Audio : MFMediaType_Video;
    inputInfo.guidSubtype = inputSubtype;

    MFT_REGISTER_TYPE_INFO outputInfo = { 0 };

    outputInfo.guidMajorType = bAudio ? MFMediaType_Audio : MFMediaType_Video;
